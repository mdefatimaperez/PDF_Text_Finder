...
*   **Python 3:** Python 3.6 or higher is recommended. You can download it from [python.org](https://www.python.org/).
...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/mdefatimaperez/PDF_Text_Finder.git
    cd PDF_Text_Finder
    ```
...
2.  **Install dependencies:**
    ...
    pip install PyPDF2
    ```
    *(Recommended: If you are using a virtual environment, make sure it's activated before running this command).*
...
1.  **Prepare your PDF file:**
    *   Place any PDF file you want to analyze in the same directory as the Python script (`pdf_lector.py`).
...
2.  **Configure the script:**
    ...
    *   **To change the keyword to search for:**
        ...
        ```python
        # Define your keyword here for easy modification
        keyword = "Shrek" # <-- Change "Shrek" here to your keyword
        keyword_lower = keyword.lower() # Lowercase version for counting

        # ... (inside the for loop) ...

        # Inside the if condition:
        if text and keyword in text: # <-- Checks for the keyword (case-sensitive)
            # ... (processing code) ...

            # Inside the counter calculation:
            # The count is made case-insensitive using lower()
            keyword_count = cleaned_text.lower().count(keyword_lower)

            # Inside the count print message:
            print(f"Occurrences of the word '{keyword}' on this page: {keyword_count}")
        ```
        *   Change the value of the `keyword` variable at the beginning of the script (or wherever you define it) to the word or phrase you want to search for.
        *   **Important:** The `keyword in text` check is case-sensitive by default. The `cleaned_text.lower().count(keyword_lower)` count is performed on the lowercased text, making it case-insensitive.

3.  **Save the changes** to the script file.

4.  **Run the script:**
    *   Open your terminal or command line.
    *   Make sure you are in the project directory (`PDF_Text_Finder`).
...
*   **Password-protected** or encrypted PDFs (PyPDF2 can handle some encryption cases if the password is provided, but this is not implemented in this basic script).
